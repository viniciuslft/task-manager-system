...
<p align="center">
  <img src="frontend/src/assets/logo-mark.svg" alt="Logo do Task Manager System" width="140" />
</p>

<p align="center">
  Um workspace full stack para gerenciar projetos, tarefas, prioridades e fluxo de entrega.
</p>
...
**Task Manager System** é uma aplicação full stack desenvolvida para gerenciar projetos e tarefas de forma clara, estruturada e escalável.

Ela combina uma API em Laravel com um frontend em Vue 3 para oferecer um workspace moderno onde os usuários podem:

- criar e gerenciar projetos
- organizar tarefas por projeto
- filtrar o trabalho por status e prioridade
- acompanhar prazos e itens em atraso
- interagir com uma interface responsiva e reutilizável

O projeto foi desenvolvido com forte foco em manutenibilidade, padrões reutilizáveis de UI, consistência da API e uma boa experiência de desenvolvimento tanto para setup local quanto para evolução futura.

---

## Prévia do Produto

Esta seção tem como objetivo apresentar os principais fluxos e a qualidade da interface da aplicação.

![Seletor de idioma](docs/screenshots/language-br.png)
![Lista de projetos](docs/screenshots/projects-list-br.png)
![Detalhes do projeto](docs/screenshots/project-details-br.png)
![Filtros de tarefas](docs/screenshots/task-filters-br.png)
...
- Listagem de projetos com status e contagem de tarefas
- Criação de novos projetos pela interface
- Navegação da lista de projetos para os detalhes do projeto
- Suporte a uma organização estruturada de projetos em um fluxo simples
...
- Atualização do status da tarefa diretamente pela interface
...
- Optimistic UI para atualização de status das tarefas
...
SQLite mantém o projeto leve e fácil de executar localmente, sem depender de um serviço externo de banco de dados.

### Vue 3 com Composition API

O frontend utiliza Vue 3 com Composition API para manter a lógica de domínio modular, reutilizável e mais fácil de escalar conforme a aplicação evolui.

### Pinia para gerenciamento de estado

Pinia foi adotado para gerenciar estado compartilhado no frontend de forma previsível e manutenível, alinhado ao ecossistema moderno do Vue.
...
Isso mantém os componentes de visualização mais limpos e fáceis de manter.

### Optimistic UI para atualização de tarefas

As mudanças de status das tarefas são refletidas imediatamente na interface e revertidas caso a requisição falhe, melhorando a responsividade sem comprometer a consistência dos dados.

### Suporte à internacionalização

O frontend está preparado para uso multilíngue com Vue I18n e arquivos de locale modulares.
...
└── docs/       # Arquivos de documentação e screenshots
```
...
- composables e gerenciamento de estado
...
### Fluxo esperado de inicialização local
...
Com os dois servidores em execução, a aplicação deve funcionar sem configuração manual adicional.
...
Esta seção é um bom lugar para mostrar como o trabalho foi estruturado do ponto de vista de gestão do projeto.
...
Isso melhora a legibilidade do histórico no Git e ajuda a conectar alterações de código aos itens de trabalho.
...
- tema escuro
- interface multilíngue
- setup local com configuração mínima
...
- ações de exclusão mais expostas no frontend
...
- interação estilo board para tarefas
...
- simplicidade no setup local
- fluxo de engenharia estruturado

Ele oferece uma base prática para gestão de projetos e tarefas, mantendo-se preparado para evoluções e novas funcionalidades.
